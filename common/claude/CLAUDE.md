...
chmodと実際の実行は分ける (&& でやらない)。 ./foo.ts のように直接実行する。
コミットの必要がないものは ./foo.ignore.ts のような命名にすることを提案する(勝手にはしない)。
...
- 「pw cli」「pw」→ プロジェクト内の `playwright-cli` MCP / ツールを最初に確認する。`npx playwright` 等のグローバルCLIではない。
